...
tsdb:
  ...
  # size-based block retention; no isolation; ingestion pauses during head
  # compaction; cardinality limits are enforced against ever-created series, not
  # live series, and tighten over time under label churn. Only affects newly
  # opened TSDBs, not ones already on disk.
