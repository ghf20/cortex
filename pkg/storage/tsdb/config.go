...
func (cfg *TSDBConfig) RegisterFlags(f *flag.FlagSet) {
	...
	f.BoolVar(&cfg.UseColumnarHead, "blocks-storage.tsdb.use-columnar-head", false, "[EXPERIMENTAL] Use the columnar head prototype instead of the real TSDB head for newly created per-tenant TSDBs. NOT for production: no size-based block retention; no isolation. Only affects newly opened TSDBs, not ones already on disk.")
