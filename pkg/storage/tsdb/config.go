...
func (cfg *TSDBConfig) RegisterFlags(f *flag.FlagSet) {
	...
	f.BoolVar(&cfg.UseColumnarHead, "blocks-storage.tsdb.use-columnar-head", false, "[EXPERIMENTAL] Use the columnar head prototype instead of the real TSDB head for newly created per-tenant TSDBs. NOT for production: only label sets of __name__ + the target labels (cluster/namespace/pod/container/node/job) + at most one extra label are accepted, other pushes fail the whole batch; no size-based block retention; no isolation; ingestion pauses during head compaction; cardinality limits are enforced against ever-created series, not live series, and tighten over time under label churn. Only affects newly opened TSDBs, not ones already on disk.")
