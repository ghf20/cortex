...
## master / unreleased
* [FEATURE] Ingester: Add experimental `-blocks-storage.tsdb.use-columnar-head` flag to use a columnar, pointerless reimplementation of the TSDB head for newly created per-tenant TSDBs, instead of the real `*tsdb.DB`. NOT for production: only label sets of `__name__` + the target labels (`cluster`/`namespace`/`pod`/`container`/`node`/`job`) + at most one extra label are accepted (other pushes fail the whole batch); no size-based block retention; no isolation; ingestion pauses during head compaction; cardinality limits are enforced against ever-created series, not live series, and tighten over time under label churn. Only affects newly opened TSDBs, not ones already on disk. #TBD
